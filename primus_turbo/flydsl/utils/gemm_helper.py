...
# Copyright (c) 2025 FlyDSL Project Contributors. Adapted from FlyDSL (https://github.com/ROCm/FlyDSL), file kernels/gemm/fp8_gemm_utils.py, modified by the Primus-Turbo team; distributed under the Apache License 2.0 (see LICENSE-APACHE), not the MIT license that covers the rest of Primus-Turbo (see LICENSE).
...
def lds_row_swizzle(row, chunks):
    """XOR key, in 16B chunks, for a `chunks`-wide LDS row, spreading one transpose-read
    column across the gfx950 LDS banks. Shared by the g2s write and transpose read sides
    (swizzle_128 / S2RLoaderTr._ptr_off) so the two cannot drift apart."""
...
class G2SLoader:
    def __init__(self, gl_src, gl_offsets, n_load_steps, lds_dtype, wave_id, chunk_stride=1024, rebase=None):
        ...
        # (arg_i8, fp8_ir_t, base_elems, num_records_bytes) instead re-bases the SRD per load: k_offset folds into the i64 base, soffset stays 0, lifting the cap at one re-base per load.
        self.rebase = rebase
    ...
    def _src_div(self, k_offset):
        ...
        # wrap to a huge unsigned SRD bound and read OOB. 0 records -> HW drops every load (matches int32 masking).
...
class ScaleBComb:
    ...
    def __init__(self, sp_tensor, dim, K, n_slabs=1, pack=1, k128p=None):
        ...
        self.PACK = pack
        # packed K-groups (PACK scales / dword); k128p overrides the stride for the variable-K wgrad's per-group packing.
        ...
        # only its valid wn groups (OOB-col clamps to 0, StoreC drops them); n_slabs>1 stacks G per-group slabs, load(slab=g) indexes group g.
...
# Cross-lane i32 primitives. gfx9 DPP: ROW_SHR|n shifts right by n in a 16-lane row; ROW_BCAST15/31 feed a row's last lane into the following rows.
_DPP_ROW_SHR = 0x110
...
class StoreCPerTensor:
    """Scalar output store shared by the per-tensor and mxfp8 GEMMs (scale None -> already
    folded into the accumulator by the scaled MMA). Output re-based per row band in 64-bit
    index (M*N > 4GB); ``col_safe`` drops the per-store OOB select, ``c_base`` retargets it."""
    ...
    def _scale(self):
        ...
        round trip instead of re-issuing both scalar loads each. Cached per MLIR block, since
        a value defined in one sibling region does not dominate a use in another."""
        if not self.scaled:
            return None
    ...
    def store(self, c_frag, base_row, base_col, col_frags=()):
        ...
        stored at ``base_col + col_delta`` in the same band (the N-quadrants of a wide tile).
        Fragment/quadrant steps ride the buffer immediate offset; a masked store keeps the add."""
        scale = self._scale()
        if self.trans:
    ...
    def _store_trans(self, c_frag, base_row, base_col, scale):
        ...
        acc[n,m], the band is pinned to the M rows and columns are N. Same buffer_store value
        math -- only the global address is transposed."""
...
class StoreCPerTensorCShuffle:
    ...
    def __init__(
        ...
        # ``double_buffer`` is the tensorwise-NT caller's alias for ``pipe``; accept both so all call sites stay valid.
        ...
        # n_tiles_b=4 -> Cc=64 -> EPL=16), emit EPL//8 back-to-back 128b stores; EPL==8 is one store.
        ...
        # pipe=True double-buffers the staging region so ti+1's ds_write overlaps ti's read+store (needs 2x LDS).
        self.pipe = pipe
        self.store_aux = store_aux
        ...
        # A_scale/B_scale optional: None -> scale already folded into the accumulator by the scaled MMA (mxfp8).
        self.scaled = A_scale is not None
        if self.scaled:
    ...
    def store(self, c_frag, base_row, base_col):
        ...
        if const_expr(self.pipe and self.n_tiles_a > 1):
            # Depth-2 softpipe over ti: prefetch ti+1's ds_write, store ti, drain WPT steps oldest-first.
...
class S2RLoaderTr:
    ...
    def load(self, lds_src, preshuffled=False, drain=True, base_off=None, vmcnt=None):
        """Return all n_tiles operand frags. Inline-asm path issues every tile's async reads
        then one trailing lgkmcnt(0); drain=False skips it when a later drain covers them.
        base_off is the double-buffer LDS parity offset; vmcnt overrides the g2s drain hint."""
...
def _robust_ab_ratio(base, cand, args, warmup=20, reps=5, iters=50):
    """Median cand/base time ratio with the two timed alternately inside one window.
    Separate windows let clock drift pick the winner (whoever runs first sees a cooler GPU);
    alternating puts both on one clock trajectory so the ratio cancels it. Assumes warm clock."""
    for _ in range(warmup):
        base(*args)
        cand(*args)
    torch.cuda.synchronize()
