...
def compile_blockscale_fp8_gemm_4w(
    *,
    ...
    128 and 192; BLOCK_N and BLOCK_K are fixed at 128.
    """
    ...
    defer_col_scale = scale_b_mode == "col1d"
    ...
    def kernel_gemm(
        ...
        tile_i, tile_j = _xcd_swizzle(num_pid_m, num_pid_n, 4)
        ...
        if const_expr(k_iters == 1):
            ...
            init_state = (
                _flat_frag(a0_frag)
                + _flat_frag(b0_frag)
                + _flat_frag((c00_frag, c01_frag, c10_frag, c11_frag))
            )
            n_a_state = n_tiles_a
            n_b_state = n_tiles_b
            ...
            if const_expr(loop_steps > 0):
                for kk, state in range(0, loop_steps, k_loop_unroll, init=init_state):
                    offset = 0
                    a0_loop = list(state[offset : offset + n_a_state])
                    offset += n_a_state
                    b0_loop = list(state[offset : offset + n_b_state])
                    offset += n_b_state
                    ...
                    loop_results = yield (_flat_frag(a0_loop) + _flat_frag(b0_loop) + _flat_frag(loop_accs))

                offset = 0
                a0_frag = list(loop_results[offset : offset + n_a_state])
                offset += n_a_state
                b0_frag = list(loop_results[offset : offset + n_b_state])
                offset += n_b_state
    ...
    def launch_gemm(
        ...
        kernel_gemm(
            ...
            A_scale,
            B_scale,
            value_attrs=make_value_attrs(2, 0, "256,256"),
        ).launch(
            grid=(grid_x, 1, 1),
            block=(256, 1, 1),
            stream=stream,
...
def compile_blockscale_fp8_gemm_8w_3stage(
    *,
    ...
    def kernel_gemm(
        ...
        num_pid_m = ceildiv(m_extent, block_m)
        num_pid_n = ceildiv(n_extent, block_n)
        tile_m, tile_n = _xcd_swizzle(num_pid_m, num_pid_n, 4)
        ...
        if const_expr(k_iters == 1):
            ...
            init_state = _flat_frag(a_frag) + _flat_frag(b_frag) + _flat_frag((c_frag,))
            ...
            if const_expr(loop_steps > 0):
                for kk, state in range(
                    ...
                    loop_results = yield (_flat_frag(a_loop) + _flat_frag(b_loop) + _flat_frag((c_loop,)))
...
def _autotune_and_run(key: tuple, candidates: list[tuple], out: torch.Tensor) -> None:
    ...
    selected_candidate = (
        next(
            ((build, args) for config, build, args in candidates if _config_key(config) == selected_key),
            None,
        )
...
def gemm_fp8_blockwise_flydsl_kernel(
    ...
    *,
    ...
    if not trans_a and trans_c:
        ...
        raise NotImplementedError(f"FlyDSL blockwise FP8 GEMM does not support trans_c=True for {layout}.")
